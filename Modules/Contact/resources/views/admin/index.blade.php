...
@section('toolbar')
    ...
    <div class="d-flex align-items-center gap-2 gap-lg-3">
        <a href="{{route('admin.contacts.export')}}" type="button" class="btn btn-sm btn-light-primary me-3"
           id="export">
...
@section('content')
    <x-admin.table :model="$contacts" search="Search In Contacts"
        ...
        <tr class="text-start text-muted fw-bold fs-7 gs-0">
            ...
            <th class="min-w-200px">{{__('Subject')}}</th>
            <th class="min-w-250px">{{__('Message')}}</th>
        ...
        @foreach($contacts as $contact)
            <tr>
                ...
                <td>
                    <a href="https://www.ipaddress.my/{{$contact->ip}}" target="_blank"> {{$contact->ip}}</a>
                </td>
                <td>
